...
% % 左右上下后前——123456
%%
round1= load ('tableup3_20211215_zengcheng.mat').sychronize;  % good 
round3= load ('tableup4_20211215_zengcheng.mat').sychronize;  % good
round5= load ('tableup1_20211215_zengcheng.mat').round1; % fine 
round7= load ('tableup2_20211215_zengcheng.mat').sychronize;
round2= load ('tableback4_20211215_zengcheng.mat').sychronize; % good
round4= load ('tableback5_20211215_zengcheng.mat').sychronize;  % fine
round6= load ('tableback1_20211215_zengcheng.mat').sychronize;
round8= load ('tableback2_20211215_zengcheng.mat').sychronize;
% round1= load ('tableup1_20211215_zengcheng_clear.mat').sychronize;  % good 
% round3= load ('tableup2_20211215_zengcheng_clear.mat').sychronize;  % good
% round5= load ('tableup3_20211215_zengcheng_clear.mat').sychronize; % fine 
% round7= load ('tableup4_20211215_zengcheng_clear.mat').sychronize;
% round2= load ('tableback1_20211215_zengcheng_clear.mat').sychronize; % good
% round4= load ('tableback2_20211215_zengcheng_clear.mat').sychronize;  % fine
% round6= load ('tableback3_20211215_zengcheng_clear.mat').sychronize;
% round8= load ('tableback4_20211215_zengcheng_clear.mat').sychronize;
% 左右上下后前——123456
all_correct_rate=[];

list_seperate_rate=[3]; list_rate_add=0:2;see_slope=0;
...
if size(train_X_xyz1,1) ~= 0
gprMdl_x1 = fitrgp(train_X_xyz1,train_Y_xyz1(:,1),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
gprMdl_y1 = fitrgp(train_X_xyz1,train_Y_xyz1(:,2),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
gprMdl_z1 = fitrgp(train_X_xyz1,train_Y_xyz1(:,3),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
...
plot(train_X_xyz1(:,1) ,ypred_f);
plot(train_X_xyz1(:,1), yci_f(:,1),'k:');
plot(train_X_xyz1(:,1), yci_f(:,2),'k:');
end

if size(train_X_xyz2,1) ~= 0
gprMdl_x2 = fitrgp(train_X_xyz2,train_Y_xyz2(:,1),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
gprMdl_y2 = fitrgp(train_X_xyz2,train_Y_xyz2(:,2),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
gprMdl_z2 = fitrgp(train_X_xyz2,train_Y_xyz2(:,3),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
...
plot(train_X_xyz2(:,1) ,ypred_f);
plot(train_X_xyz2(:,1), yci_f(:,1),'k:');
plot(train_X_xyz2(:,1), yci_f(:,2),'k:');
end

if size(train_X_xyz3,1) ~= 0
gprMdl_x3 = fitrgp(train_X_xyz3,train_Y_xyz3(:,1),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
gprMdl_y3 = fitrgp(train_X_xyz3,train_Y_xyz3(:,2),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
gprMdl_z3 = fitrgp(train_X_xyz3,train_Y_xyz3(:,3),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
%% predict from the older mdl
figure(43)
[ypred_f, ~, yci_f] = predict(gprMdl_y3, train_X_xyz3 );

title('no data, not much confidence on the right');
hold on;
plot(train_X_xyz3(:,2) ,gprMdl_y3.Y,'r.');
plot(train_X_xyz3(:,2) ,ypred_f);
plot(train_X_xyz3(:,2), yci_f(:,1),'k:');
plot(train_X_xyz3(:,2), yci_f(:,2),'k:');

end

if size(train_X_xyz4,1) ~= 0
gprMdl_x4 = fitrgp(train_X_xyz4,train_Y_xyz4(:,1),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
gprMdl_y4 = fitrgp(train_X_xyz4,train_Y_xyz4(:,2),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
gprMdl_z4 = fitrgp(train_X_xyz4,train_Y_xyz4(:,3),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
%% predict from the older mdl
figure(44)
[ypred_f, ~, yci_f] = predict(gprMdl_y4, train_X_xyz4 );

title('no data, not much confidence on the right');
hold on;
plot(train_X_xyz4(:,2) ,gprMdl_y4.Y,'r.');
plot(train_X_xyz4(:,2) ,ypred_f);
plot(train_X_xyz4(:,2), yci_f(:,1),'k:');
plot(train_X_xyz4(:,2), yci_f(:,2),'k:');

end



if size(train_X_xyz5,1) ~= 0
gprMdl_x5 = fitrgp(train_X_xyz5,train_Y_xyz5(:,1),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
gprMdl_y5 = fitrgp(train_X_xyz5,train_Y_xyz5(:,2),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
gprMdl_z5 = fitrgp(train_X_xyz5,train_Y_xyz5(:,3),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
%% predict from the older mdl
figure(45)
[ypred_f, ~, yci_f] = predict(gprMdl_z5, train_X_xyz5 );

title('no data, not much confidence on the right');
hold on;
plot(train_X_xyz5(:,3) ,gprMdl_z5.Y,'r.');
plot(train_X_xyz5(:,3) ,ypred_f);
plot(train_X_xyz5(:,3), yci_f(:,1),'k:');
plot(train_X_xyz5(:,3), yci_f(:,2),'k:');
end



if size(train_X_xyz6,1) ~= 0
gprMdl_x6 = fitrgp(train_X_xyz6,train_Y_xyz6(:,1),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
gprMdl_y6 = fitrgp(train_X_xyz6,train_Y_xyz6(:,2),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
gprMdl_z6 = fitrgp(train_X_xyz6,train_Y_xyz6(:,3),'KernelFunction','squaredexponential','ComputationMethod','v','ActiveSetMethod','likelihood','OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('AcquisitionFunctionName','expected-improvement-plus'),'HyperparameterOptimizationOptions', struct('UseParallel',true));
%% predict from the older mdl
figure(46)
[ypred_f, ~, yci_f] = predict(gprMdl_z6, train_X_xyz6 );

title('no data, not much confidence on the right');
hold on;
plot(train_X_xyz6(:,3) ,gprMdl_z6.Y,'r.');
plot(train_X_xyz6(:,3) ,ypred_f);
plot(train_X_xyz6(:,3), yci_f(:,1),'k:');
plot(train_X_xyz6(:,3), yci_f(:,2),'k:');
end
%% seeee~
...
%     each_test_file_Y_left=[each_test_file_Y_left - each_test_file_Y_left(1,:)];    
        ...
        elseif each_intent == 3
             ...
             each_test_file_left_draw=each_test_file_left(:,2);
        ...
        elseif each_intent == 5
             ...
             each_test_file_left_draw=each_test_file_left(:,3);
...
% 
%            %% predict from the older mdl
%                 figure(46+count_fuck)
%                 [ypred_f, ~, yci_f] = predict(gprMdl_draw, temp_combine_X ); % new_added part
%                 count_fuck=count_fuck+1;
%                 title('no data, not much confidence on the right');
%                 hold on;
%                 plot(temp_combine_X_draw ,gprMdl_draw.Y,'r.'); % 红的是真实的
%                 plot(temp_combine_X_draw ,ypred_f,'b.'); % 蓝的是预测的
%                 plot(temp_combine_X_draw, yci_f(:,1),'k:');
%                 plot(temp_combine_X_draw, yci_f(:,2),'k:');
%                 size(gprMdl_draw.Y,1)
%                 size(ypred_f,1)
%                 test_x=[each_test_file_left(1:end-1,1:3) each_intent*ones(size(each_test_file_left,1)-1,1)];
%                 
%                 [ypred_test, ~, yci_test] = predict(gprMdl_draw, test_x );
%                 plot(each_test_file_left_draw(1:end-1,:) ,each_test_file_left_draw(2:end,:) ,'r+'); % 青色是测试真实的
%                 plot(each_test_file_left_draw(1:end-1,:) ,ypred_test,'b+');  % 绿色是测试预测的
%                 plot(each_test_file_left_draw(1:end-1,:), yci_test(:,1),'m:');
%                 plot(each_test_file_left_draw(1:end-1,:), yci_test(:,2),'m:');        
%                 
%                 max_x=max([temp_combine_X_draw; each_test_file_left_draw;])
%                 min_x=min([temp_combine_X_draw; each_test_file_left_draw;])
%                 plot([max_x,min_x],[max_x,min_x],'y')
...
ALL_corr=[ALL_corr this_round_corr];
ALL_MSE=[ALL_MSE all_MSE_this_rate];
ALL_result=[ALL_result; all_result_this_rate; -1*ones(1,6);];

ALL_result_use=[ALL_result_use; all_result_this_rate_use; -1*ones(1,4);];
    ...
    name_fig=['saveas(88,"save' num2str(seperate_rate) '.jpg");' ];
    eval(name_fig); 
    
  buyao0= find(all_result_this_rate_use(:,4)  ==0 );
  all_result_this_rate_use(buyao0,:)=[];
  
  
corr_total=find(all_result_this_rate_use(:,3) == all_result_this_rate_use(:,4));
sb=length(corr_total);
rate=sb/size(all_result_this_rate_use,1);

all_correct_rate=[all_correct_rate rate];
