...
t0=toc; % calculate initial time
    k_cartesian_7 = diag([100,100,80,100,100,100,50]);
    b_cartesian_7 = diag([150,150,120,100,100,50,80]*1.2/100);   
    
    %% natural admittance control
    % cartesian impedance controller parameters
    k_cartesian = diag([100,100,100]);
    b_cartesian = diag([100,100,100]/100);
    % equivalent inertial in joint space
    H_inv = diag([1 1 1 1 1 1 0]*1000);
...
%     iiwa.realTime_startImpedanceJoints(0.6,00,0.1,3500,250,10);
    time=toc;
    dt=time-t0;
%     alldt=[alldt dt];
    dt=Ts;
...
%         k=8/1000; 
%         if i <=1000
%             contact_force_after=[k*i k*i 0].';
%         else
%             contact_force_after=[8-k*(i-1000) 8-k*(i-1000) 0].';
%         end
        contact_force_after=[0 4 0].';
        adjust=[adjust contact_force_after];
        
%         f_attractor = k_cartesian*(end_effector_p-target_end_effector_p)+b_cartesian*(v_cartesian-v_cartesian_target);
                f_attractor = k_cartesian*(end_effector_p-target_end_effector_p+J_dx_dq_this*target_joint_velocity*dt)...
         +b_cartesian*(v_cartesian-v_cartesian_target+(-J_dx_dq_this*target_joint_velocity+J_dx_dq_next*target_joint_velocity_next));
     
        
        
        all_f_attractor=[all_f_attractor f_attractor];
        
%         all_f_attractor adjust;
...
%         end
        ...
        t = t+dt;
         ...
         [ pose_end, nsparam_end, rconf_end, jout_end ] = ForwardKinematics( final_target_joint_position2, robot_type );
