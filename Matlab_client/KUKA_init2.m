...
% This example script is used to gather kinect, EMG(7+2) and KUKA for no control


close all;clear;clc;
warning('off')

coef_LASSO=[0;-712881.177851848;940435.984640465;2167685.07689852;-133837.354838006;-3083828.34876321;1099871.03969526;759160.463014564;379897.878700495;-159150.857617996;0.545841214944539;0.438854161218699;-0.974731551550650];
tercept_LASSO=-2.658458894109285;
...
fuck=[-0.463647609000806,0.820126730911849,0,-1.17997019621945,0,1.14149572645849,2.67794504458899]
...
disp(['未打开右边的！',datestr(now)])
fopen(t_server);%打开服务器，直到建立一个TCP连接才返回；
disp(['打开了右边的！',datestr(now)])
%get(t_server);
data_all = [];count_right = 0;
    

%get(t_server);
data_all = [];count_self = 0;
%代表那个大循环   
...
if maxx_jian < 0.004
    ...
    if abs(now_predict_x) < 0.6
        predict_x_count=1;
    end
     if abs(now_predict_z) < 0.6
        predict_z_count=1;
    end   
     if abs(now_predict_y) < 0.6
        predict_y_count=1;
     end
    ...
    predict_y=predict_y+now_predict_y;
                    ...
                    if abs(predict_x)<1
                        out_predict_x = 0;
                    elseif predict_x > 0
                        out_predict_x = predict_x - 1;
                    elseif predict_x < 0
                        out_predict_x = predict_x + 1;                        
                    end
                    out_all_predict_x=[out_all_predict_x out_predict_x];   
                    if abs(predict_y)<1
                        out_predict_y = 0;
                    elseif predict_y > 0
                        out_predict_y = predict_y - 1;
                    elseif predict_y < 0
                        out_predict_y = predict_y + 1;                        
                    end
                    out_all_predict_y=[out_all_predict_y out_predict_y];                 
                    if abs(predict_z)<1
                        out_predict_z = 0;
                    elseif predict_z > 0
                        out_predict_z = predict_z - 1;
                    elseif predict_z < 0
                        out_predict_z = predict_z + 1;                        
                    end
                    out_all_predict_z=[out_all_predict_z out_predict_z];                    
...
% oo=[ contact_force_after; this_frame_self(2:end-1); out_predict_x; out_predict_y; out_predict_z];
    oo=[this_frame_self(2:end-1); out_predict_x; out_predict_y; out_predict_z];
...
anat=[[sychronize(1+3,:)- sychronize(2+3,:)];  [sychronize(3+3,:)- sychronize(4+3,:)];  [sychronize(5+3,:)- sychronize(7+3,:)]];  

% [A1,PS]=mapminmax([sychronize(4:end-3,:); anat;]);
% all_F=sychronize(end-2:end,:);
all_F=sychronize(1,:)+sychronize(2,:)+sychronize(3,:);

% after_all_EMG=[[sychronize(4:end-3,:); anat;]; sychronize(end-2:end,:);  ].';
% [coef_1,fitinfo] = lasso(after_all_EMG,all_F,'Weights',ones(size(after_all_EMG,1),1),'Alpha',0.75,'Lambda',0.005);
% pre_y=after_all_EMG*coef_1+fitinfo.Intercept;
% figure(90);plot(pre_y); hold on; plot(all_F)
% legend('predict force','real')
% R2=1-sum((all_F-pre_y.').^2)/sum((all_F-mean(all_F)).^2)

% rate = max(abs(all_F_contact(1,:)))/
...
% [A1,PS]=mapminmax(sychronize(4:end,:));
% after_all_EMG=A1.';
left_x=after_all_EMG(:,7)-after_all_EMG(:,5);
left_y=after_all_EMG(:,1)-after_all_EMG(:,2);
left_z=after_all_EMG(:,3)-after_all_EMG(:,4);
% 
% kill=[];
% for m = 1:7
%     this=after_all_EMG(:,m);
%     jj=[];
%     for q = 1:length(this)
%         neww=this(q)+0.5;
%         if neww<0
...
% max_x=max(all_contact_force_after(1,:));where_x=find(all_contact_force_after(1,:)==max_x);
% max_y=max(all_contact_force_after(2,:));where_y=find(all_contact_force_after(2,:)==max_y);
% max_z=max(all_contact_force_after(3,:));where_z=find(all_contact_force_after(3,:)==max_z);
% rate_x=max_x/left_x(where_x);
% rate_y=max_y/left_y(where_y);
% rate_z=max_z/left_z(where_z);
% predict_x=left_x*rate_x;
% predict_y=left_x*rate_y;
% predict_z=left_x*rate_z;
% % figure; plot(all_contact_force_after(1,:));hold on; plot(predict_x);
% 
% figure; plot(all_contact_force_after(1,:));hold on; plot(predict_x);
% figure; plot(all_contact_force_after(2,:));hold on; plot(predict_y);
% figure; plot(all_contact_force_after(3,:));hold on; plot(predict_z);
% figure; plot(all_contact_force_after(1,:));hold on;plot(after_all_EMG(:,7)*100);hold on; plot(after_all_EMG(:,5)*100);
% legend('F','EMG1','EMG2')
...
% after_all_EMG=kill.';
% new_y=abs((after_all_EMG(:,2)+after_all_EMG(:,6))/2-after_all_EMG(:,1));
% new_x=abs(after_all_EMG(:,7)-after_all_EMG(:,5));
% new_z=abs(after_all_EMG(:,3)-after_all_EMG(:,4));
% new_all=abs(new_y)+abs(new_x)+abs(new_z);
% figure; plot(new_x);hold on; plot(new_y);hold on; plot(new_z);
% legend('x','y','z')
% all_F=abs(all_F_contact(1,:))+abs(all_F_contact(2,:))+abs(all_F_contact(3,:));
% 
% 
% after_all_EMG=kill.';
% [coef_1,fitinfo] = lasso(after_all_EMG,all_F,'Weights',ones(size(after_all_EMG,1),1),'Alpha',0.75,'Lambda',0.005);
...
% 
% figure(23); plot(all_F);hold on; plot(new_all*10);
% legend('real','predict')
% 
% 
% hh=[new_all(1:1222)*4; new_all(1223:2145)*5; new_all(2146:end)*1.4;];
% figure(24); plot(all_F);hold on; plot(hh*10);
% legend('real','predict')
% h=hh.';
% R2=1-sum((all_F-h).^2)/sum((all_F-mean(all_F)).^2)
