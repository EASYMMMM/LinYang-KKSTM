function [data cartis_obs]= get_init_space_maze(input)
% 开始折磨队友，目前的pose有
...
data = cell(108,4);die_dueto_col=0;
...
delta_y789=0.10;delta_x789=0.05;delta_h789_1=0.4;delta_h789_2=0.2;
%% y x z  还是循环写起来好一些。。。我他妈的真是烦
...
for j = 1:6
    ...
    data{3+(j-1)*11,2}(2)=delta_blank;
    data{5+(j-1)*11,2}(2)=delta_blank;
    data{7+(j-1)*11,2}(2)=delta_blank;
    data{9+(j-1)*11,2}(2)=delta_blank;
...
for j = 1:6
    data{4+(j-1)*11,2}(2)=delta_y456;
    data{6+(j-1)*11,2}(2)=delta_y456;
    data{8+(j-1)*11,2}(2)=delta_y456;
end

for j = 1:11
    data{0*11+j,2}(1)=delta_blank;
    data{2*11+j,2}(1)=delta_blank;
...
elseif input == 2221
cartis_obs=[2+11+66 2+11*3 4+11*4+66 8+11*4 10+11*3+66 10+11];
end
