...
%现在位置在一个地方
%现在加入EMGIMU
%standardize_IMU.mat, 上下上下上，停，前后前后，停，左右左右
...
ux=[0;28.4404944950314;0;-62.1770404834889;0;89.3824650214797;0]*pi/180;
theta_points=ux.';
...
splict=100;
            all_a23=[all_a23 splict];
            all_b23=[all_b23 splict];
            all_y23=[all_y23 splict];
            all_a31=[all_a31 splict];
            all_b31=[all_b31 splict];
            all_y31=[all_y31 splict];            
            all_a21=[all_a21 splict];
            all_b21=[all_b21 splict];
            all_y21=[all_y21 splict];
...
while FINISH == 0
    ...
    STOP=0;
          ...
          count_up=0;
            ...
            now_pos_jj=[0.6; 0; 0.525;]; % 认为自己一直在这里
            [total_act way_points which_state_now myspace]= RL2m3m3_table(now_pos_jj);
            if total_act == 11
                FINISH=1
                break
            end
...
%     points=[0.6;0;0.525;];
    ...
    yd=points(2,i_inv);
    zd=points(3,i_inv);
    [All_theta] = inverse_with_gesture_table(xd,yd,zd,init_theta1,init_theta2).';
...
%   [v_direction]= which_refer_v(total_act12); % 这是干嘛的
 ...
 if over == 0
    ...
    if over == 0
        ...
        target_joint_position_next=theta(:,1+1).';
        target_joint_velocity=target_joint_velocity_next;
        target_joint_velocity_next=theta_dot(:,1+1);
    else
        target_joint_position=target_joint_position_next;
...
% time update
        [Jac_this,A_mat_products] = Jacobian_table(target_joint_position,robot_type);
...
%         % controller
        [Jac,A_mat_products] = Jacobian_table(feedback_joint_position,robot_type);
        J_dx_dq = Jac(1:3,:);
...
figure;plot(data_all_IMU(14,:))
figure;plot(data_all_IMU(15,:))
figure;plot(data_all_IMU(16,:))

figure;plot(data_all_IMU(39,:))
figure;plot(data_all_IMU(40,:))
figure;plot(data_all_IMU(41,:))

figure;plot(data_all_IMU(65,:))
figure;plot(data_all_IMU(64,:))
figure;plot(data_all_IMU(66,:))
