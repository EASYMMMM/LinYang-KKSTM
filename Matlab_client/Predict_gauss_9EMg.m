...
figure(1);
plot(result(:,end)); hold on; plot(y_train_z(train_start:train_stop,:)); hold on;
plot(result_yci(:,end-1),'k--')
plot(result_yci(:,end),'k--')
legend('predict z','True value z','Prediction interval limits');

figure(2);
plot(result(:,end-1)); hold on; plot(y_train_y(train_start:train_stop,:)); hold on;
plot(result_yci(:,end-2),'k--')
plot(result_yci(:,end-3),'k--')
legend('predict y','True value y','Prediction interval limits');

figure(3);
plot(result(:,end-2)); hold on; plot(y_train_x(train_start:train_stop,:)); hold on;
plot(result_yci(:,end-4),'k--')
plot(result_yci(:,end-5),'k--')
