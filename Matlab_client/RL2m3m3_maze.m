function [q,R,series_state via_points which_state space cartis_obs OBSTACLE]= RL2m3m3_maze(where_robot_3,last_space,CHANGE,lastq,lastR,num_obs,slope_IMU,OBSTACLE)
...
%         plot3(all_space(node,1),all_space(node,2),all_space(node,3),'y.'); hold on;
%     else
%         plot3(all_space(node,1),all_space(node,2),all_space(node,3),'b.'); hold on;
%     end
% end
% xlabel('x')
% ylabel('y')
...
if CHANGE == 0
    ...
    if num_obs == 0
        ...
        for i=1:66
            R(i,i+66)=-3; %double up
            R(i,i+66*3)=-2; %H up R stay
            R(i,i+66*2)=-1; %R up H stay
            if i-11>0
                R(i,i-11)=-1;%forward
            end
            if i+11<66+1
