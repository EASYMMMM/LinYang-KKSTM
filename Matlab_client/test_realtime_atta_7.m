...
t0=toc; % calculate initial time
    k_cartesian_7 = diag([100,100,100,100,100,100,100]/10*8);
    b_cartesian_7 = diag([100,100,100,100,100,100,100]*1*0.707*9/1000);   
    H_inv = diag([1 1 1 1 1 1 1]*1000*0.4);
    w_n=(k_cartesian_7.*H_inv)^0.5;
    w_n=w_n(1,1)
    zeta=b_cartesian_7.*H_inv/2./w_n;
    zeta=zeta(1,1)
    syms xxx; %定义x是一个未知量
    ...
    t = 0;
    list_x=[];list_v=[];list_a=[];all_qe=[];all_qed=[];all_qedd=[];all_flagg=[];
   %%
   coef_v_order9=[
...
%         k=max/1000; 
%         if i <=1000
%             contact_force_after=[k*i 0 0].';
%         else
%             contact_force_after=[max-k*(i-1000) 0 0].';
...
%%
       ...
       qe=(-target_joint_position+feedback_joint_position).'+target_joint_velocity*dt; %1 7=>7 1 
       qde=-target_joint_velocity+feedback_joint_velocity_after+target_joint_dotdot*dt;
       
        q_t1dd=H_inv*(J_dx_dq'*contact_force_after-k_cartesian_7*qe-b_cartesian_7*qde);
        equal_F=pinv(J_dx_dq.')*(-k_cartesian_7*qe-b_cartesian_7*qde);
        all_f_attractor=[all_f_attractor equal_F];
     qdetjia=qde+q_t1dd*dt;
     
     qetjia=qe+qde*dt+0.5*dt*dt*q_t1dd;
     all_qe=[all_qe qetjia];

        
        goal=target_joint_position_next+qetjia.';
