function data = get_init_space(input)
tic
...
for p = 1:24
    if p <= 12
        ...
        p_h=data{p-12,2}(3)+data{p,2}(3)/2;
    end
...
%     catch
%         [All_theta] = [];
%         disp('at the begin, there is no solution')
% %         data{k,4}=1;
% %         continue
%     end
...
%                 catch
%                 [All_theta] = [];
%                 catch_die=catch_die+1;
%                 end
% 
%                 if count_no > 40
%                     data{k,4}=1;
%                     die_duetoout=die_duetoout+1;
%                     break
%                 end
%             end
...
%     All_theta=All_theta.'*pi/180;
%     [no, len_way_points]=size(All_theta);
%     count_fea=0;
%     for each_sol = 1:len_way_points
%         what=All_theta(:,each_sol);
%         what(7)=-pi/8;
%         point_collision=what;
% 
%         whether_col = check_collision(point_collision);
% 
%         if whether_col == 1
%             count_fea=count_fea+1;
%         end
%     end
%     if count_fea == len_way_points
%         data{k,4}=1;
%         die_dueto_col=die_dueto_col+1;
%     else
%         data{k,4}=0;
%     end
    
  
    %     plot3(now_x,now_y,now_z,'b.','MarkerSize',2);hold on;
end
