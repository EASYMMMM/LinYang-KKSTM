...
over=0;pre_y=0; KEEP = 0;temp_intent=[];temp_EMG_left=[];WAIT=0;break_GPR=0;HELP=0;temp_output=[];
for i_theta=1:1000
%%    control  
    i_theta;
    if i_theta == 1
...
%% 计算角度与速度            
    ...
    count_y=count_y+1;
        ...
        temp_EMG_left=[temp_EMG_left; EMG_left;];
           ...
           if length(temp_output)>=2
            if temp_output(end) == 2 && temp_output(end-1) == -1
                count_up=count_up+1
            end
           end
           
    if count_up > 0 && up == 0 && NEVER <= 1 && laststate <=6 && DOWN == 0
...
%% y rot
        if end_effector_p(3) > 0.285
               ...
               a_ydof=h_ydof*(Tory+k_ydof*y1*pi/180-d_ydof*v_6(6));
...
figure(24)
plot(all_end_effector_p(1,5771:6091)-0.5415); hold on;
plot(all_y_pro(5771:6091)/100)
all_refer;


figure(28)
plot(all_EMG(2,:)); hold on;
plot(all_EMG(4,:))