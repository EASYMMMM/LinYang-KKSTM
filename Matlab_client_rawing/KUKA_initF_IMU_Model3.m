...
%% load GPR
file_GPR1 = 'gprMdl20220127_1_1to1.mat';
gprMdl_x1_1=load(file_GPR1).gprMdl_x1;
gprMdl_x2_1=load(file_GPR1).gprMdl_x2;

% gprMdl_y2_1=load(file_GPR1).gprMdl_y2;
% gprMdl_z2_1=load(file_GPR1).gprMdl_z2;
% gprMdl_y1_1=load(file_GPR1).gprMdl_y1;
% gprMdl_z1_1=load(file_GPR1).gprMdl_z1;

file_GPR2 = 'gprMdl20220127_2_1to1.mat';
gprMdl_x1_2=load(file_GPR2).gprMdl_x1;
gprMdl_x2_2=load(file_GPR2).gprMdl_x2;

% gprMdl_y2_2=load(file_GPR2).gprMdl_y2;
% gprMdl_z2_2=load(file_GPR2).gprMdl_z2;
% gprMdl_y1_2=load(file_GPR2).gprMdl_y1;
% gprMdl_z1_2=load(file_GPR2).gprMdl_z1;
...
while FINISH == 0
    ...
    STOP=0;
          ...
          if count_up>15 && NEVER == 0 && laststate <=6
            ...
            % send to GPR
            if laststate <= 6
                % which_state_now<=3 this_intent = 1
                if which_state_now == 1
                    this_intent = 1;
                    in_step=14;
                elseif which_state_now == 2
                    this_intent = 1;
                    in_step=25;
                elseif which_state_now == 4
                    this_intent = 2;
                    in_step=14;
                elseif which_state_now == 5
                    this_intent = 2;
                    in_step=25;
                end   
                if which_state_now == 2 || which_state_now == 5
                    start_point=way_points(1,1);
                elseif which_state_now == 1 || which_state_now == 4
                    start_point=way_points(2,1)/sin(angle_comb);
                end
                fwrite(t_server_GPR,[88888.888,start_point,this_intent,in_step],'double');%第一个数据头，然后当前点，然后意图
            end
...
%在这之前，是正常的，如果breakGPR=1了，说明打断并且立即重新规划。
    end           
    ...
    theta_d_des=zeros(7,lie);

    
    % GPR 导致的更新
    if break_GPR == 1
               % 待定
        target_end_point3=[target_end, target_end, way_points(3,2)]
        way_points(:,2)=target_end_point3;
        theta_d_des(:,1)=current_v;
    end
...
if break_GPR == 1
    T_tot=ceil(T_tot*100-i_theta)/100;
    GPR_T_tot=T_tot
end
...
contact_force_after(3)=0-level*3;
...
%% 收到GPR了吗   
    if  t_server_GPR.BytesAvailable>0
        ...
        break_GPR = 1;
        target_end=traj_GPR(length(tvec) - i_theta);
        current_v=target_joint_velocity;
        disp('GPR break')
        break
    else
        break_GPR = 0;
