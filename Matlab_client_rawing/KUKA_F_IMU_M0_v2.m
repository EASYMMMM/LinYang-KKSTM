...
addpath('C:\Lin YANG\from me\KUKA\KUKA_Matlab\KST-Kuka-Sunrise-Toolbox-master')
...
if DOWN == 1 || UP == 1 || up == 1
    ...
    processed_way_points=[mean(way_points,2) mean(way_points,2)];
...
if UP == 1 || DOWN == 1
    contact_force_after(3)=0;
elseif BOTTOM == 0
    contact_force_after(3)=-EMG1;
elseif BOTTOM == 1 && FUCKact(end) == 3
    if HELP == 1
        contact_force_after(3)=-EMG7/8/2;
    else
        contact_force_after(3)=-EMG7/4/2;
    end
elseif BOTTOM == 1 && FUCKact(end) == -3
    if HELP == 1
        contact_force_after(3)=-EMG5/8/2;
    else
...
% contact_force_after(1)=contact_force_after(1)*1.5;
% contact_force_after(2)=contact_force_after(2);
...
%      if v_filt-last_v_filt>0.1
%          v_filt=last_v_filt+0.1;
%      elseif v_filt-last_v_filt<-0.1
%          v_filt=last_v_filt-0.1;
%      end
...
figure(57)
plot(all_target_end_effector_p(end,:)); hold on;
plot(all_contact_force_after(3,:)/10); hold on;
plot(new_f(3,:)/1000); hold on;
plot(all_end_effector_p(end,:))
legend('target','all_contact_force_after','DIRECT F','all_end_effector_p')
