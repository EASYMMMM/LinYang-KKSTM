...
% [U,S,V] = svd(J_dx_dq)
        ...
        k=max/150; 
        if i <=1000 && i>850
            my_torque(end)=[k*(i-850)];
        elseif i >1000 && i<=1900
            my_torque(end)=[max];
        elseif i >1900 && i<=2050
            my_torque(end)=[max-k*(i-1900)];
%             contact_force_after=[0 0 0].';
        end
%         contact_force_after=[8 0 0].';
        adjust=[adjust; my_torque;];   
...
%        xde=-J_dx_dq*target_joint_velocity+v_filt;
    ...
    feedback_joint_velocity_after=pinv(Jac)*[v_filt; 0; 0; 0;];
       ...
       d_7dof=0.1;
       h_7dof=10;
           w_n=(k_7dof.*h_7dof)^0.5;

    zeta=d_7dof*h_7dof/2/w_n

    
       a_7dof=h_7dof*(my_torque(end)-k_7dof*this_p(end)-d_7dof*feedback_joint_velocity_after(end));
