...
if STEP == 2
    rotate_matrix_H=[ [cos(angle_comb), -sin(angle_comb) 0]; [sin(angle_comb), cos(angle_comb) 0]; [0 0 1];];
    new_chuyi=rotate_matrix_H.'*all_end_effector_p(:,6000:8000);
    neww=new_chuyi;
% chuyi=all_end_effector_p(2,6000:8000)./sin(angle_comb);
...
for file = start:2
  ...
  %%  method 1
            ...
            for shit = 1:length(v)-1
                ...
                temp_X=[temp_X [round_this(1,shit); intent_t;]];
                temp_Y=[temp_Y [round_this(1,shit+1); intent_t;]];
                if this*next < 0
                    hh_X=round_this(1,shit+1).*ones(1,40);
                    hh_Y=round_this(1,shit+1).*ones(1,40);
...
for g = 1:2
        ...
        eval(expr_g);
        expr_g2=['train_X_xyz' num2str(g) ' = all_round_train_X(which' num2str(g) ',1);'];
        eval(expr_g2);
        expr_g3=['train_Y_xyz' num2str(g) ' = all_round_train_Y(which' num2str(g) ',1);'];
        eval(expr_g3);
end
...
%     each_test_file_Y_left=[each_test_file_Y_left - each_test_file_Y_left(1,:)];    
        ...
        result = myGRP_onlyx(each_test_file_left(:,1:end-1), each_intent, new_gprMdl_1);
...
%         [result, nmb] = check_GPR(train_X_xyz2(:,1:end-1), each_intent, new_gprMdl_1,new_gprMdl_2,new_gprMdl_3);
        ...
        result_xyz=result(:,1);
        real_xyz=each_test_file_Y_left(1:end,1);
    ...
    all_result_this_rate=[all_result_this_rate; this_round_result; zeros(1,2);];
...
rate2=sb2/size(this_round_result_use,1);
    ...
    kk=[all_result_this_rate_use(:,1) zeros(length(all_result_this_rate_use(:,1)),2)];
    huilai=rotate_matrix_H*kk.';
...
if STEP ==2    
    plot(huilai(1,:)); hold on; plot(all_end_effector_p(1,6000:8000));
    
save('gprMdl20220127_2_1to1.mat','gprMdl_x1','gprMdl_x2')
end

if STEP ==1 
save('gprMdl20220127_1_1to1.mat','gprMdl_x1','gprMdl_x2')
end
