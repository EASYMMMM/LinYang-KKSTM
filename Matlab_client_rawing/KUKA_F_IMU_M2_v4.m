...
% figure;plot(all_refer(1,:)); hold on;
% plot(all_refer(2,:)); hold on;
% plot(all_refer(3,:)); 
...
%     end
    ...
    if this_point_after >= size(theta_dot,2)
        ...
        if total_act(1)-total_act(2) == -3 && output(end) == 2 && (all_end_effector_p(1,end) < all_end_effector_p(1,end-1))
                this_point_after = size(theta_dot,2);
                come_in=[ come_in 1];
        elseif total_act(1)-total_act(2) == 3 && output(end) == 3 && (all_end_effector_p(1,end) > all_end_effector_p(1,end-1))
                this_point_after = size(theta_dot,2);
                come_in=[ come_in 2];
        else
            come_in=[come_in 4];
...
%         all_pre_z=[all_pre_z pre_z];
    ...
    if EMG1 >= 0.12 || DOWN == 1 || up == 1
           ...
           elseif outclass == 3 || F_filt(1) > 10
            ...
            F_filtered_int=F_filtered_int/4;
...
if UP == 1 || DOWN == 1
    contact_force_after(3)=0;
elseif BOTTOM == 0
    contact_force_after(3)=-2.5-EMG1;
elseif BOTTOM == 1 && FUCKact(end) == 3
    if HELP == 1
        contact_force_after(3)=-EMG7/4;
    else
        contact_force_after(3)=-EMG7/2;
    end
elseif BOTTOM == 1 && FUCKact(end) == -3
    if HELP == 1
        contact_force_after(3)=-EMG5/4;
    else
        contact_force_after(3)=-EMG5/2;
    end
else
    contact_force_after(3)=0;
end
...
% %
% figure(32)
% plot(new_f(3,7368:8246)/10); hold on;
% plot(all_EMG(8,7368:8246)); hold on;
% plot(all_EMG(6,7368:8246)); hold on;
% plot(all_end_effector_p(1,7368:8246))
% legend('F','EMG7','EMG5','Px')
% kalmanFZ = dsp.KalmanFilter('ProcessNoiseCovariance', 0.0001,...
%     'MeasurementNoiseCovariance',R,...
%     'InitialStateEstimate',5,...
%     'InitialErrorCovarianceEstimate',1,...
%     'ControlInputPort',false); %Create Kalman filter
% dsds=kalmanFZ(all_contact_force_after(3,:));
% figure(45)
% plot(all_contact_force_after(3,:)); hold on;
% plot(dsds)
%% analyze 
...
figure(34)
plot(all_end_effector_p(end,:)); hold on;
plot(all_target_end_effector_p_next(end,:))
