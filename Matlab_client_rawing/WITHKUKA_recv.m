...
t0=toc; % calculate initial time
  ...
  coef_f_order26=[
    ...
    0.02960040617057,  0.02718719168633
             ...
             new_f=[];new_v=[];new=[];
    all_feedback_joint_position=[];all_predict_y=[];all_predict_x=[];all_predict_z=[];
%% Control loop   
...
all_dt_real=[];all_new_EMG=[];all_new_EMG2=[];
q_t1dd=theta_dotdot(:,1);q_t1d=[0 0 0 0 0 0 0].';q_t1=[0 0 0 0 0 0 0].';intergal_goal=[];adjust=[];all_str=[];
pause(10);
...
if tcp_rece.BytesAvailable > 0
   str_rece = fscanf(tcp_rece); 
   str_rece2=str2double(str_rece);
   all_str = [all_str str_rece2];
end
...
%                         each_theta = pi/2-mean(delta_theta);
    ...
    all_new_EMG=[all_new_EMG; new_EMG;];
                    ...
                    if delta_wan_y > 0.04 || delta_zhou_y > 0.04  % only motion of wrist related to elbow
                        predict_y=predict_y+1;
                    elseif delta_wan_y < -0.04 || delta_zhou_y < -0.04
                        predict_y=predict_y-1;
                    else
                        predict_y_count=1;
                    end
                    ...
                    if delta_zhou_z > 0.04 || delta_jian_z > 0.04
                        predict_z=predict_z+1;
                    elseif delta_zhou_z < -0.04 || delta_jian_z < -0.04
                        predict_z=predict_z-1;
                    else
                        predict_z_count=1;
                    end
    ...
    time=toc;
    dt_real=time-t0;
    all_dt_real=[all_dt_real dt_real];      %%%%%%%%%%%%%%%%%%%%%%%%%%% TIME
    dt=Ts;
    t0=toc;
    counter=counter+1;
