...
feedback_joint_velocity_after=zeros(7,1);all_output=[];last_space=0;
...
angle_7=0;up=0;
last_state=8;all_real_point=[];MDZZ=[];all_refer_pos=[];laststate=8;BIGTIME=4;
...
while FINISH == 0
    round=round+1;
          contact_force_after=[0; 0; 0;];v_filt=[0;0;0;];delete_point=1;accu_point=1;diminish=0;
            ...
            [total_act way_points which_state_now myspace]= RL2m3m3(now_pos_3,angle_7,up,lock,last_space,laststate);
            ...
            total_act
            % 判断是否会停下
            if last_state == which_state_now && round >1 
                break
            else
...
all_refer_pos=[all_refer_pos refer_pos];
...
%         projection=end_effector_r*contact_force';
 ...
 if end_effector_p(3) < 0.15
    ...
    target_joint_position_3_ori(3)=target_joint_position_3_ori(3)-0.001;
    target_joint_velocity_3_ori(3)=target_joint_velocity_3_ori(3)-0.00001;
