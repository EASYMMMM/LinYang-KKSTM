...
while FINISH == 0
    ...
    STOP=0;
          ...
          count_up=0;
            ...
            total_act 
            % 判断是否会停下
            if last_state == which_state_now && up == 0
...
for i_inv = 2 : lie
    init_theta1=180;
    init_theta2=0;
    if which_state_now <=6
        xd=(way_points(1,1)+way_points(1,2))/2;
        yd=(way_points(2,1)+way_points(2,2))/2;
    else
...
%  if v_direction*contact_force_after(1)<0
 ...
 if over == 0
    ...
    if which_state_now <=6
        target_joint_position_next=theta(:,end).'; 
        target_joint_velocity_next=theta_dot(:,end);
    else
        if over == 0
