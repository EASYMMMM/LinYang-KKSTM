...
new_torque=[new_torque; cell2mat(my_t);];
    ...
    oo=[ F_contact; this_frame_self(2:end-1);];
...
% max_x=max(all_F_contact(1,:));where_x=find(all_F_contact(1,:)==max_x);
% max_y=max(all_F_contact(2,:));where_y=find(all_F_contact(2,:)==max_y);
% max_z=max(all_F_contact(3,:));where_z=find(all_F_contact(3,:)==max_z);
...
% figure; plot(all_F_contact(1,:));hold on; plot(predict_x);
% 
% figure; plot(all_F_contact(1,:));hold on; plot(left_x*100);
% figure; plot(all_F_contact(2,:));hold on; plot(left_y*100);
% figure; plot(all_F_contact(3,:));hold on; plot(left_z*100);
% figure; plot(all_F_contact(1,:));hold on;plot(after_all_EMG(:,7)*100);hold on; plot(after_all_EMG(:,5)*100);
% legend('F','EMG1','EMG2')
...
%     
% else
%  temp_EMG=each_update_16;  % 假设 x:125  y 3456 z 3457
% % end
%  each_update_16=[];
