...
while 1
    ...
    if MY_COUNT == judge2    %静息值采集 0-5000  提取数据
            ...
            std_step1=std(use3(lie11:lie12,:));  %方差 最小值
...
%         y_EMG_afterfilter=y_EMG_afterfilter(end-10:end,:);
    ...
    if MY_COUNT == judge3  % 峰值采集结束 （绷紧手臂 5000 - 15000
        %% gather STD
            ...
            std_step2=std(use_max(lie1:lie2,:));  %方差 峰值
            
            new_guiyidata2=[ std_step1; std_step2;];  %归一化
            [tttww, ps]=mapminmax(new_guiyidata2.',0,1);  % 归一化
...
%%        STEP 2
    if ~isempty(each_update_16)
        ...
        each_update_16=[];
        to_EMG=to_EMG+size(current_all,1);
        all_9xyz(from_EMG:to_EMG,:)=current_all;  %全部数据
        from_EMG=from_EMG+size(current_all,1);  %all_9xyz 左边指针
...
%%         STEP 3   开始EMG数据传输
    if ~isempty(each_update_16)
        ...
        m_all=median(current_all);  %取中位数
        this_std=std(current_all);    %方差
        map_int_new=mapminmax('apply',this_std.',ps).';  %归一化
